    ...
    def write_qihe_file(self, filename, footprints, layer, X_Offset, Y_Offset,
        ...
        try:
            with open(filename, 'w', newline='') as file:
                ...
                for mod in filtered_footprints:
                    ...
                    if mod.GetAttributes() & (pcbnew.FP_EXCLUDE_FROM_POS_FILES | pcbnew.FP_EXCLUDE_FROM_BOM):
                        ...
                        if self.verbosity_level >= 3:
                            debug_info += " - Excluded by attribute"
                            self.log_activity(debug_info)
                        continue  # Exclude by attribute takes precedence

                    if self.is_excluded(mod, exclude_patterns):
                        excluded_by_pattern.append(mod)
                        if self.verbosity_level >= 3:
                            debug_info += " - Excluded by pattern"
                            self.log_activity(debug_info)
                        continue
                    ...
                    if self.is_priority(mod, priority_patterns):
                        ...
                        nozzle_num, feeder_location = component_mapping[mod.GetValue()]  # Get mapping details
                        if self.verbosity_level >= 3:
                            debug_info += f" - Mapped N:{nozzle_num}, F:{feeder_location}"
                            self.log_activity(debug_info)
                ...
                self.log_activity(
                    f"- Excluded (pattern/attribute): {len(excluded_by_pattern)} / {len(excluded_by_attribute)}")
            ...
            error_msg = (
                f"Permission Error: Unable to write to file {filename}. "
                f"The file may be open in another application.: {str(e)}"
            )
            self.log_activity(error_msg)
            log_message(error_msg, log_type="ERROR")

        except Exception as e:
            error_msg = f"Unexpected error occurred while writing to file {filename}: {str(e)}"
            self.log_activity(error_msg)
            log_message(error_msg, log_type="ERROR")
